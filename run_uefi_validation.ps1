...
if ($selectorCount -gt 1) {
    throw 'Select only one of -Tiny, -FirstFrame, or -Frames.'
}
if ($Frames -lt 0) {
...
if ($Frames -eq 0 -and -not $Tiny -and -not $FirstFrame -and -not $Continuous) {
    $Continuous = $true
}
...
$isContinuousValidation = [string]::IsNullOrEmpty($diagnosticMode)
...
if ($SerialLog) {
    ...
    $serialName = [System.IO.Path]::GetRelativePath($PSScriptRoot, $serialPath)
} else {
    ...
    while ($true) {
        ...
        if ($content -and $content -ne $lastContent) {
            ...
            $faultMatches = [regex]::Matches(
                $content,
                '(?im)(CONTINUOUS_DESKTOP_FAULT=[^\r\n]*|CPU_FAULT_[A-Z_]+|PANIC:|UEFI_FRAME_FAULT_CONTEXT)')
...
if ($status -in @('FAULT', 'QEMU_EXITED', 'TIMEOUT_NO_PROGRESS')) {
    exit 1
}
exit 0

...
unsafe class Program {
    ...
    private const bool UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C1 = false;
    private const bool UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C2 = false;
    private const bool UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C3 = false;
    private const bool UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C4 = false;
    private const bool UEFI_PROBE_CURSOR_PNG_STANDALONE_S4D0 = false;
    ...
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREMETA = false;
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_NOOP = false;
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_BYTES_NOOP = false;
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_ZLIB_HEADER = false;
    ...
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_BYTES = false;
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_INLINE = false;
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_BOUNDARY = false;
    private const bool UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_CONTEXT = false;
    ...
    private static UefiCursorPngProbeStage GetUefiCursorPngStandaloneProbeStage() {
        ...
        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C1) {
            return UefiCursorPngProbeStage.S4C1;
        }

        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C2) {
            return UefiCursorPngProbeStage.S4C2;
        }

        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C3) {
            return UefiCursorPngProbeStage.S4C3;
        }

        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C4) {
            return UefiCursorPngProbeStage.S4C4;
        }

        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4D0) {
            return UefiCursorPngProbeStage.S4D0;
        }

        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4D1) {
            return UefiCursorPngProbeStage.S4D1;
        }

        if (UEFI_PROBE_CURSOR_PNG_STANDALONE_S4D2) {
            return UefiCursorPngProbeStage.S4D2;
        }
    ...
    private static void CursorPngLoadWrapperProbe() {
        ...
        if (UEFI_PROBE_CURSOR_PNG_LOAD_AFTER_IHDR) {
        ...
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREMETA) {
            probeMode = PngLoader.LoadProbeMode.DecompressPreMeta;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_NOOP) {
            probeMode = PngLoader.LoadProbeMode.DecompressNoop;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_BYTES_NOOP) {
            probeMode = PngLoader.LoadProbeMode.DecompressBytesNoop;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_ZLIB_HEADER) {
            probeMode = PngLoader.LoadProbeMode.DecompressZlibHeader;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_OUTPUT_ALLOC) {
            probeMode = PngLoader.LoadProbeMode.DecompressOutputAlloc;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_DEFLATE_HEADER) {
            ...
            probeMode = PngLoader.LoadProbeMode.DecompressPrepNoop;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_METADATA) {
            probeMode = PngLoader.LoadProbeMode.DecompressPrepMetadata;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_BYTES) {
            probeMode = PngLoader.LoadProbeMode.DecompressPrepBytes;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_INLINE) {
            probeMode = PngLoader.LoadProbeMode.DecompressPrepInline;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_BOUNDARY) {
            probeMode = PngLoader.LoadProbeMode.DecompressPrepBoundary;
        } else if (UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_CONTEXT) {
            probeMode = PngLoader.LoadProbeMode.DecompressPrepContext;
        ...
        try {
            ...
            if (UEFI_PROBE_CURSOR_PNG_NOOP_LOADER) {
                ...
                ProbeNormalDesktopCursorSourcePng();
                       ...
                       UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C1 ||
                       UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C2 ||
                       UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C3 ||
                       UEFI_PROBE_CURSOR_PNG_STANDALONE_S4C4 ||
                       UEFI_PROBE_CURSOR_PNG_STANDALONE_S4D0 ||
                ...
                ProbeNormalDesktopCursorSourcePng();
                       ...
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREMETA ||
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_NOOP ||
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_BYTES_NOOP ||
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_ZLIB_HEADER ||
                       ...
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_BYTES ||
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_INLINE ||
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_BOUNDARY ||
                       UEFI_PROBE_CURSOR_PNG_DECOMPRESS_PREP_CONTEXT ||

...
namespace {
    enum : u64 {
        VmReserve = 0x20, VmCommit = 0x21, VmProtect = 0x22,
        VmRelease = 0x23, VmQuery = 0x24, TlsInitialize = 0x30,
        TlsCurrentBlock = 0x31, FlsAlloc = 0x32, FlsGet = 0x33,
        FlsSet = 0x34, FlsCleanup = 0x35, ThreadId = 0x40,
        ThreadStackBounds = 0x41, ThreadRuntimeState = 0x42,
        MonotonicTicks = 0x50, MonotonicFrequency = 0x51,
        SystemTimeNs = 0x52, RandomBytes = 0x60, ProcessExit = 0x70,
        FailFast = 0x71,
    };
...
u64 guidexos_pal_abi_version(void) { return Call(1); }
u64 guidexos_pal_service_request(u64 request, u64 requestLength) {
    return Call(5, request, requestLength);
}
...
void* guidexos_pal_tls_current_block(void) { return (void*)Call(TlsCurrentBlock); }
i32 guidexos_pal_fls_alloc(void (*cleanup)(void*)) { return (i32)Call(FlsAlloc, (u64)cleanup); }
void* guidexos_pal_fls_get(i32 slot) { return (void*)Call(FlsGet, (u64)(long long)slot); }
i32 guidexos_pal_fls_set(i32 slot, void* value) { return (i32)Call(FlsSet, (u64)(long long)slot, (u64)value); }
void guidexos_pal_fls_cleanup(void) { (void)Call(FlsCleanup); }

u64 guidexos_pal_thread_id(void) { return Call(ThreadId); }
i32 guidexos_pal_thread_stack_bounds(void** low, void** high) {
    return (i32)Call(ThreadStackBounds, (u64)low, (u64)high);
}
void* guidexos_pal_thread_runtime_state(void) { return (void*)Call(ThreadRuntimeState); }
u64 guidexos_pal_monotonic_ticks(void) { return Call(MonotonicTicks); }
u64 guidexos_pal_monotonic_frequency(void) { return Call(MonotonicFrequency); }
u64 guidexos_pal_system_time_ns(void) { return Call(SystemTimeNs); }

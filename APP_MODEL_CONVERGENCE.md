...
## 31. Phase 17 separate managed user-image artifact and loader design gate

Phase 17 is **Outcome D + Outcome E with bounded Outcome C prerequisites**.
The independent artifact and the host-side bounded loader contract are now
real and reproducible, but the design-approval rule correctly stops kernel
loader integration when the smallest generated payload proves that private
TLS/FLS, GC/runtime state, exception support, and a guideXOS PAL/import
policy are required.

The new `UserManagedProof/guideXOS.UserManagedProof.csproj` has no kernel
project reference. Its source is only `Program.Main() => 42`; it has no
strings, arrays, reflection, dynamic loading, hardware, filesystem, network,
threading, or ABI calls. Its pinned build is SDK `10.0.401` with
`rollForward=disable`, target `net9.0`/`win-x64` AMD64, and explicit
`Microsoft.DotNet.ILCompiler` `9.0.0`. It requests size optimization,
trimming, deterministic output, no debug symbols, a map file, fixed image base
`0x0000401000000000`, and `/fixed`. Two clean publishes produced the same
SHA-256, so the user artifact is distinct from the old `net7.0` kernel image
and reproducible on the checked-in toolchain contract.

The inspected artifact is PE32+ AMD64, 886,784 bytes, SHA-256
`2ab3f8394b20c35889a5a3073bf9b322b5d79ddc447910ad1fe7a6eadc4b7ec8`, with
entry RVA `0x985b0` and absolute entry `0x00004010000985b0`. Its five sections
are `.text` RX, `.rdata` R, `.data` RW, `.pdata` R, and `.rsrc` R. No section
is RWX. There is no base-relocation directory, so the first image policy is
fixed-base rather than a general relocation engine.

The exact local JSON manifest is generated by
`Tools/inspect_managed_image.py`; it records section raw/file ranges,
permissions, PE directories, all 131 imports across 11 Windows/CRT DLLs,
relocation types, TLS, and the NativeAOT `map.xml` evidence. No guideXOS
kernel symbol is imported. The external imports are nevertheless not a
guideXOS ABI: the image includes KERNEL32 FLS/thread/process/virtual-memory
imports, ADVAPI32, bcrypt, ole32, and CRT imports. They require a reviewed
PAL/import policy before a user image can execute in guideXOS.

The PE has a TLS directory with a 272-byte template. The NativeAOT map has 13
TLS/thread-static entries, 43 GC/frozen-object entries, 355 writable-static
entries, 31 ReadyToRun helpers, 3,530 total map records, and explicit metadata,
module, type-manager, module-initializer, GC-static, frozen-object, and
ReadyToRun structures. `.pdata` contains 30,276 bytes of unwind data; the map
contains 55 exception-handling records and the image imports exception/unwind
entry points. The generated payload therefore classifies as:

```text
TLS/FLS       PRIVATE_TLS_FLS_REQUIRED
GC            PRIVATE_GC_REQUIRED
Exceptions    UNWIND_METADATA_AND_EXCEPTION_RUNTIME_REFERENCED
Startup       MODULE_METADATA_GC_STATICS_FROZEN_OBJECTS_READYTORUN
```

This is exactly the evidence the Phase 16 audit was missing. It rules out
reusing kernel statics, kernel heap, kernel thread state, or the current
direct-import runtime. The smallest source payload is not a no-runtime
artifact once stock NativeAOT startup and support objects are linked.

`Tools/phase17_loader_probe.py` derives the bounded segment contract from the
real PE and models a process-private user mapping. It validates the complete
image before mapping, maps RX/R/RW sections, zero-fills the RW tail, checks
the entrypoint, rejects RWX, and releases all simulated pages. Four generations
completed with 20 segments mapped/reclaimed, 972 image pages allocated/
reclaimed across the four-lifetime cohort, 438,272 zero-fill bytes proven, and zero
outstanding mappings. Eleven malformed cases were rejected before mapping:
format, segment count, overlap, entrypoint, file bounds, memory/file size,
kernel VA, stack/user boundary, BSS overflow, RWX, and TLS metadata.

This is a host-side contract proof, not a managed-entry or kernel-QEMU proof.
The existing kernel loader remains the freestanding native proof loader. No
...
thread transition contract; private GC/heap/write-barrier state or a proven
no-GC runtime pack; exception/unwind policy; fixed-width startup metadata and
native-first trampoline; ABI stubs for System Information and Exit; and the
kernel-side bounded PE/managed-image descriptor, validator, RX/R/NX mapper,
